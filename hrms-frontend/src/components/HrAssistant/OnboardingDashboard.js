...
import { Container, Row, Col, Button, Badge, Table, Card, Alert, Modal } from 'react-bootstrap';
...
const OnboardingDashboard = () => {
  ...
  const [onboardingSubtab, setOnboardingSubtab] = useState('Document Submission');
    ...
    const submissions = documents
      ...
      .map((doc) => {
        ...
        return {
          ...
          document_type: doc.submission.document_type || doc.document_type || doc.document_key,
  ...
  const computeApplicantDocStatus = useCallback((doc) => {
    ...
    if (documentKey) {
      ...
      requirement = documentRequirements.find((req) => req.document_key === documentKey);
    ...
    const rawStatus = submission?.status || requirement?.status || (requirement ? 'not_submitted' : null);

    const normalizedStatus = (rawStatus || '').toLowerCase();



    const statusLabelFromBackend = submission?.status_label || requirement?.status_label || null;

    const statusDescription = submission?.status_description || requirement?.status_description || '';
    ...
    const fallbackLabels = {
      ...
      neutral: requirement
        ...
        ? (requirement.is_required ? 'Not Submitted' : 'Optional')
    ...
    const statusLabel = statusLabelFromBackend || fallbackLabels[statusVariant];



    const submissionStatus = normalizedStatus
  ...
  const calculateStats = () => {
    ...
    const stats = {
      ...
      accepted: applicants.filter(a => a.status === 'Accepted' || a.status === 'Offer Accepted').length,
  ...
  const getStatusBadge = (status, showIcon = true) => {
    ...
    const statusConfig = {
      ...
      'Offer Accepted': { color: 'success', icon: faUserPlus, text: 'Offer Accepted', customClass: 'status-accepted' },
  ...
  const handleViewDetails = (record) => {
    ...
    if (record.status === 'Offered') {
      ...
      setJobOfferData({
        ...
        department: record.jobPosting?.department || record.job_posting?.department || '',

        position: record.jobPosting?.position || record.job_posting?.position || '',

        salary: record.jobPosting?.salary_min && record.jobPosting?.salary_max 
  ...
  const fetchAllApplicantsDocumentStatus = async () => {
    ...
    for (const applicant of onboardingApplicants) {
      ...
      try {
        ...
        if (requirementsResponse.data.success && submissionsResponse.data.success) {
          ...
          if (requiredDocs.length === 0) {
            ...
            const pendingSubmissions = submissions.filter(
              ...
              (submission) =>
                ...
                (submission.status === 'pending' || submission.status === 'pending_review' || submission.status === 'uploaded')
            ...
            if (approvedSubmissions.length === requiredIds.length) {
              ...
              latestDate = approvedSubmissions
                ...
                .map((submission) => submission.reviewed_at || submission.submitted_at)
  ...
  const reviewDocumentSubmission = async (submissionId, status, reason = '', documentKey = null) => {
  ...
  const openApproveModal = (submission, doc) => {
    ...
    const derivedDocumentKey =
      ...
      (submission.document_requirement_id ? `requirement_${submission.document_requirement_id}` : null);

    setApproveModalData({
      submissionId: submission.id,
      documentTitle: doc?.title || submission.document_requirement?.document_name || 'this document',
  ...
  const confirmRejectDocument = async () => {
    ...
    if (window.confirm('Are you sure you want to reject this document? The employee will be notified to re-upload.')) {
      ...
      await reviewDocumentSubmission(rejectingSubmissionId, 'rejected', rejectionReason, rejectingDocumentKey);
    ...
    <div className="row gy-3">
      ...
      {docs.map((doc) => {
        ...
        if (submission) {
          ...
          requirementBadgeLabel = requirement.is_required ? 'Required' : 'Optional';

          requirementBadgeVariant = requirement.is_required ? 'danger' : 'secondary';
                ...
                <div className="mt-3 d-flex flex-column flex-lg-row align-items-lg-center gap-2">
                  ...
                  <div className="ms-lg-auto d-flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => submission && openApproveModal(submission, doc)}
                    ...
                    <Button
                      ...
                      onClick={() => submission && handleRejectDocument(submission)}
  ...
  ), [computeApplicantDocStatus, deleteDocumentRequirement, handleRejectDocument, openApproveModal, openSubmissionFile, reviewingDocument, statusBadgeStyles]);
  ...
  const allDocumentsApproved = () => {
    ...
    return requiredDocs.every(req => {
      ...
      return submission && (submission.status === 'received' || submission.status === 'approved');
  ...
  const handleOfferJob = (applicant) => {
    ...
    setJobOfferData({
      ...
      salary: applicant.jobPosting?.salary_min && applicant.jobPosting?.salary_max 

        ? `₱${applicant.jobPosting.salary_min.toLocaleString()} - ₱${applicant.jobPosting.salary_max.toLocaleString()}`

        : (applicant.job_posting?.salary_min && applicant.job_posting?.salary_max
                    ...
                    {(() => {
                      ...
                      const filtered = onboardingApplicants.filter((applicant) => {
                        ...
                        const matchesSearch =
                          ...
                          !searchTerm || nameVariants.some((value) => value.includes(searchTerm));
                              ...
                              <tbody>
                                  ...
                                  {filtered.map((applicant) => {
                                    ...
                                    const applicantEmail =
                                      ...
                                      applicant.applicant?.email || applicant.employee_email || 'N/A';
                                    ...
                                    <td style={{ padding: '16px', verticalAlign: 'middle' }}>
                                          ...
                                          {position && (
                                            ...
                                            <div
                                              ...
                                              style={{ color: '#6c757d', fontStyle: 'italic', fontSize: '0.9rem' }}
                  ...
                  >
                    ...
                    <div className="d-flex align-items-center" style={{ gap: '12px' }}>
                      ...
                      <div className="flex-grow-1 d-flex align-items-center" style={{ gap: activeTab === 'Overview' ? '0.2in' : '14px' }}>
                        ...
                        {activeTab !== 'Overview' && activeTab !== 'Pending' && activeTab !== 'Shortlisted' && activeTab !== 'Interview' && activeTab !== 'Offered' && activeTab !== 'Accepted Offer' && activeTab !== 'Onboarding' && (
              ...
              <div className="modal-body p-4">
                          ...
                          <div className="row g-3">
                            ...
                            <div className="col-md-8 d-flex align-items-center">
                              ...
                              <div className="form-check">
                                ...
                                <label className="form-check-label" htmlFor="assistant-additional-doc-required">
                        ...
                        <div className="card-body">
                          ...
                          {loadingFollowUpRequests ? (
                                    ...
                                    return (
                                      ...
                                      <tr key={request.id}>
                                        ...
                                        <td>
                                          ...
                                          <div className="fw-semibold">{request.applicantName || 'Applicant'}</div>
                            ...
                            <div className="text-center text-muted py-5">
                              ...
                              <p className="mb-0 small">Applicants can submit follow-ups when they need to clarify or update their documents.</p>
                ...
                {approveModalData && (
                  <Modal
                    ...
                    backdrop="static"
                  >
                    <Modal.Header closeButton={!reviewingDocument} className="border-0">
