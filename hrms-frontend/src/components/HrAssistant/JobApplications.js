...
const JobApplications = () => {
  ...
  const handleAuthError = (error) => {
    if (error.response?.status === 401) {
      console.log('[HrAssistant-JobApplications] Authentication failed - token may be invalid or expired');
      ...
      console.log('[HrAssistant-JobApplications] Access denied - insufficient permissions');
      showError('Access denied. You do not have permission to perform this action.');
      return true;
    } else if (error.response?.status === 404) {
      console.log('[HrAssistant-JobApplications] API endpoint not found');
  ...
  const fetchApplications = async () => {
    try {
      ...
      console.log('🔍 [HrAssistant-JobApplications] Fetching applications...');
      
      const response = await axios.get('http://localhost:8000/api/applications', {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      console.log('📊 [HrAssistant-JobApplications] Applications fetched:', response.data.length);
      setApplications(response.data);
      calculateStats(response.data);
      
    } catch (error) {
      console.error('❌ [HrAssistant-JobApplications] Error fetching applications:', error);
  ...
  const updateApplicationStatus = async (applicationId, newStatus) => {
    try {
      ...
      console.log(`🔄 [HrAssistant-JobApplications] Updating application ${applicationId} to: ${newStatus}`);
      ...
      if (response.status === 200) {
        ...
        console.log(`✅ [HrAssistant-JobApplications] Status updated successfully`);
      }
      
    } catch (error) {
      console.error('[HrAssistant-JobApplications] Error updating status:', error);
      if (!handleAuthError(error)) {
  ...
  const downloadResume = async (applicationId, applicantName) => {
    try {
      ...
      console.log('📄 [HrAssistant-JobApplications] Downloading resume for application:', applicationId);
      ...
      console.log('✅ [HrAssistant-JobApplications] Resume downloaded successfully');
      
    } catch (error) {
      console.error('❌ [HrAssistant-JobApplications] Error downloading resume:', error);
      if (!handleAuthError(error)) {
  ...
  return (
    <div className="container-fluid p-4">
      ...
      {/* Statistics Cards - Original Height for HR Assistant */}
      <Row className="mb-4">
        <Col md={2}>
